...
namespace Sandbox
{
    public static class DpiAwareApi
    {
        ...
        public static Dpi SystemDPI
        {
            get
            {
                ...
                try
                {
                    ...
                    {
                        dpi.X = Gdi32.GetDeviceCaps(desktopDC, DeviceCap.LOGPIXELSX);
                        dpi.Y = Gdi32.GetDeviceCaps(desktopDC, DeviceCap.LOGPIXELSY);
                    }
                }
                catch (Exception)
                {
        ...
        public static Dpi GetDpiForWindow(IntPtr hwnd, MONITOR_DPI_TYPE dpiType = MONITOR_DPI_TYPE.MDT_DEFAULT)
        {
            ...
            var monitor = User32.MonitorFromWindow(hwnd, MonitorOptions.MONITOR_DEFAULTTONEAREST);
        ...
        public static PROCESS_DPI_AWARENESS GetProcessDpiAwareness(IntPtr hprocess)
        {
            ...
            catch (DllNotFoundException)
            {
                try
                {
                    ...
                    awareness = PInvoke.User32.IsProcessDPIAware() ? PROCESS_DPI_AWARENESS.PROCESS_SYSTEM_DPI_AWARE : PROCESS_DPI_AWARENESS.PROCESS_DPI_UNAWARE;
                }
        ...
        public static int LogicalMonitorScaleFactor(IntPtr hwndMonitor)
        {
            try
            {
                ...
                if (User32.GetMonitorInfo(hwndMonitor, ref monitorInfo))
                {
                    int logicalDesktopWidth = User32.GetSystemMetrics(SystemMetric.SM_CXVIRTUALSCREEN);
        ...
        public static bool RegisterAsDpiAware(bool perMonitorAwareness = true, bool fallBackToSystemAwareness = true)
        {
            if (HasPerMonitorDpiSupport)
            {
                var result = SHCore.SetProcessDpiAwareness(PInvoke.PROCESS_DPI_AWARENESS.PROCESS_PER_MONITOR_DPI_AWARE);
                ...
                {
                    return PInvoke.User32.SetProcessDPIAware();
                }
            }
            else if (fallBackToSystemAwareness)
            {
        ...
        public static bool SetProcessDpiAwareness(PROCESS_DPI_AWARENESS value)
        {
            ...
            catch (DllNotFoundException)
            {
                try
                {
                    ...
                    return value == PROCESS_DPI_AWARENESS.PROCESS_SYSTEM_DPI_AWARE ? PInvoke.User32.SetProcessDPIAware() : false;
                }
                catch (EntryPointNotFoundException)
                {
                }
